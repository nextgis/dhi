'''Prepare DHI data from stacks of TIFs. To run:
   python grass-averages-fpar.py x:\MCD15A2\ x:\MCD15A2\combined\fpar8\ y:\dhi\global\fpar_8\combined-v2\ fpar8
...
numslices = len(glob.glob(str(years[0]) + '/tif-lai-qa/' + '*.tif'))

for year in years:
    i = 0
    for f in glob.glob(str(year) + '/tif-lai-qa/' + '*.tif'):
        i+=1
...
#Calculate median values for N slices
for i in range(1,numslices+1):
    list = ''
    for year in years:
        list = list + ',' + str(year) + '_' + str(i)
    ...
    grass.run_command('r.series', input=list, output=str(i) + '_med', method='median')
...
grass.mapcalc('dh3_' + t + ' = std' + t + '/ave' + t)
